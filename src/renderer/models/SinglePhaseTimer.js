...
export class SinglePhasTimer extends EventTarget {
    ...
    static #RemainingTimeLabelElement = document.querySelector(
        ".time-remaining-label"
    );
    static #NameLabelElement = document.querySelector(".name-label");
    static #RemainingTimeIndicatorElement = document.querySelector(
    ...
    adjustElapsedTime(adjustmentAmount) {
        if (this.#status !== TimerStatus.Inactive) {
            this.#elapsedTime = Math.max(
                this.#elapsedTime + adjustmentAmount,
                0
            );
            this.#draw();
        }
    ...
    setKeyBindingInfo(providedKeyBindingInfo) {
        ...
        const newKeyBindingInfo = defaultKeyBindings.map(
            (defaultKeyBinding) => {
                const providedKeyBinding = providedKeyBindingInfo.find(
                    binding =>
                        binding.actionName === defaultKeyBinding.actionName
                );
    ...
    #draw() {
        ...
        const secondsRemaining = Math.max(
            Math.round(this.timeRemaining / Second),
            0
        );
        SinglePhasTimer.#RemainingTimeIndicatorElement.setAttribute(
            "stroke-dashoffset",
            `${Math.max((100 - progressPercent) * circumference / 100, 0)}px`
        );
        SinglePhasTimer.#RemainingTimeLabelElement.textContent =
            secondsRemaining;
    }
